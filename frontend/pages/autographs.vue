...
import { useSchemaOrg } from 'nuxt-schema-org/schema';
...
const crates = computed(() => {
  ...
  return data.value.filter((c) => c.type === 'Autograph Capsule').reverse();
});
...
useSeoMeta({
  ...
  ogSiteName: title,
  ogUrl: `https://case.oki.gg/autographs`,
  ogType: 'website',
  author: 'Oki',
});
...
const jsonld = {
  ...
  mainEntity: {
    '@type': 'ItemList',
    itemListElement: crates.value.slice(0, 10).map((crate, index) => ({
      '@type': 'ListItem',
      position: index + 1,
      item: {
        '@type': 'Product',
        ...
        category: 'CS2 Autograph Capsule',
        offers: {
          '@type': 'Offer',
  ...
  breadcrumb: {
    ...
    itemListElement: [
      ...
      },
      {
        '@type': 'ListItem',
        position: 2,
        name: 'Autograph Capsules',
        item: 'https://case.oki.gg/autographs',
      },
    ],
  },
  isPartOf: {
